/**
 * Forked from @deepseek-ai/dsh-client-ui-workspace/src/client/WorkspaceBrowser.tsx
 * (MIT, deepseek-ai/deepseek-harness). The workspace/session browsing region:
 * section header (title + view options + add workspace), search, the grouped
 * tree or flat list, and the workspace dialogs.
 *
 * Fork adaptations (see plugin/README.md「继承原版会话面板」):
 *   - Registered into the shell's `sidebar.panel` list slot (id 'sessions')
 *     instead of the `sidebar.workspaces` single hole; self-gates on the
 *     shell's activePanelId via the SessionsPanelHost wrapper in index.ts.
 *   - The directory-flow hole is renamed to this package's own
 *     `sidebar.panel.sessions.directoryFlow` (the upstream
 *     `sidebar.workspaces.directoryFlow` is already declared by the built-in
 *     ui-workspace — one declarer per slot).
 *   - Workspace rows gain a 「复制路径」menu entry (copyPath action, wired in
 *     WorkspaceBrowser/rows/Rows.tsx) that copies the folder path.
 */
            ...
            >
              <ProjectRowItem
                ...
                actions={group.workspaceId === undefined
                  ...
                  : {
                    copyPath: () => {
                      if (group.cwd !== undefined) writeClipboard(group.cwd)
                    },
                    rename: () => {
    ...
    <div className={clsx(css.root, !wide && css.rail)}>
      <div className={css.sectionHeader}>
        ...
        <WorkspacePickFlow
          ...
          renderDirectoryFlow={owner => renderSlot('sidebar.panel.sessions.directoryFlow', owner)}
          addOnly
          side="right"
          onPick={(workspaceId) => {
            setWsPickerOpen(false)
