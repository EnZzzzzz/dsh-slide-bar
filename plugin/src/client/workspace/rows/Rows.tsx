/**
 * Forked from @deepseek-ai/dsh-client-ui-workspace/src/client/rows/Rows.tsx
 * (MIT, deepseek-ai/deepseek-harness). Workspace browser tree row components:
 * pure presentational — all data and callbacks arrive via props. Row ... menus
 * are visual-only except workspace Rename/Delete/CopyPath and session
 * Rename/Fork/Archive. Fork adaptation: the workspace (folder) menu gained a
 * 「复制路径」entry (copyPath) on top of the upstream Rename/Delete.
 */
...
export function ProjectRowItem({ group, onToggle, onCreate, actions, drag, t }: {
  ...
  actions?: { rename: () => void; delete: () => void; copyPath: () => void } | undefined
  ...
  const workspaceMenuItems = [
    { id: 'copyPath', label: t('menu.copyPath'), icon: <IconCopyOutline16 /> },
        ...
        {actions !== undefined && (
          <Menu
            ...
            onSelect={(id) => {
              ...
              if (id === 'copyPath') { actions.copyPath(); return }
